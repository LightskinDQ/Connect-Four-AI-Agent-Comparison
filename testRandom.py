...
#tests everything the same as in test.py but now the game has a random factor in it so there is a 25% chance of a piece of either player dropping after 4 turns

def random_move(board, depth=4):
    """Random move selection"""
    return randomConnectFour.random_valid_column(board)
...
algorithms_dict = {
    ...
    'Gemini': gemini_move, #comment out gemini if output taking too long
}
...
def simulate_game(move_fn_a, move_fn_b, depth_a=4, depth_b=4, max_moves=100):
    ...
    """
    board = randomConnectFour.createBoard()
    ...
    for x in range(max_moves):
        ...
        current_piece = randomConnectFour.PLAYER_PIECE if turn == 0 else randomConnectFour.AI_PIECE
        ...
        try:
            ...
            # Validate move
            if not randomConnectFour.isValidMove(board, col):
                # Invalid move = automatic loss
                return "B" if turn == 0 else "A"
            
            # Make the move
            row = randomConnectFour.getNextRow(board, col)
            randomConnectFour.playMove(board, row, col, current_piece)
            
            # Check for win
            if randomConnectFour.checkWinningMove(board, current_piece):
                return "A" if turn == 0 else "B"
            
            # Check for draw (board full)
            if not any(randomConnectFour.EMPTY in row for row in board):
                return "Draw"
    ...
    for rows, cols in board_sizes:
        ...
        for alg_name, alg_func in algorithms.items():
            ...
            for trial in range(trials):
                ...
                board = randomConnectFour.createBoard(rows, cols)
                ...
                for i in range(moves_to_make):
                    ...
                    if randomConnectFour.isValidMove(board, col):
                        row = randomConnectFour.getNextRow(board, col)
                        # Alternate pieces deterministically
                        piece = randomConnectFour.PLAYER_PIECE if i % 2 == 0 else randomConnectFour.AI_PIECE
                        randomConnectFour.playMove(board, row, col, piece)
...
def create_performance_graphs(tournament_results, scalability_results, node_performance):
    ...
    for alg in ['Random', 'Minimax', 'AlphaBeta', 'ExpectiMinimax', 'Gemini']:
        execution_times[alg] = []
    ...
    # 2. Node Expansion Comparison (Single Move)
    ...
    plt.suptitle('Random Connect 4 AI Algorithm Performance Analysis', fontsize=16, fontweight='bold', y=0.98)
    
    # Save the figure
    plt.savefig('random_connect4_performance_analysis.png', dpi=300, bbox_inches='tight')
    print("Performance analysis graphs saved as 'random_connect4_performance_analysis.png'")
...
if __name__ == '__main__':    
    # Run tournament
    TRIALS = 3  # Number of games per matchup
    ...
    test_board = randomConnectFour.createBoard()
